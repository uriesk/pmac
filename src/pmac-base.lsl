// PARAMOUR MULTI-ANIMATION CONTROLLER (PMAC) CORE v2.5
// by Aine Caoimhe (c. LACM) March 2015-July 2017
...
// *********************************************************
string defaultGroup = "Standing";     // name of a group (not the full card name!) to load by default regardless of its permission setting
...
integer autoOn = FALSE;               // TRUE = will start in auto mode after a reset
...
string gs_ConfigName = ".PMAC-CONFIG";    // Name of optional notecard with user defined configuration that overide the script's values if present
...
integer silent = FALSE;               // TRUE = PMAC will only say critical information to chat...usually only errors will be seen (*** NEW IN PMAC 2,0***)
// FALSE = PMAC will notify users of some changes
integer allowRemoteControl = FALSE;   // TRUE = allow the menu to be accessed by a non-user (please see instructions about this feature!)(*** NEW IN PMAC 2.5***)
// ***************************************
...
handleRemoteCall(string message){
    ...
    if (command == "PMAC_REMOTE_QUIT"){
        if (myState == "RUNNING") doQuit(); // has to be nested
    } else if (command == "PMAC_REMOTE_SYNCH") {
        if (myState == "RUNNING") doSynch(); // has to be nested
    } else {
        user = NULL_KEY;
...
showEditMenu() {
    ...
    integer u;
    while (u<llGetListLength(positions)){
    ...
    butDia = []+["-","DEL ANIM","SWAP ANS","< PREV","SYNCH","NEXT >","REVERT THIS","STORE THIS","STORE ADDON","EDIT OFF","SAVE CARD","SAVE NEW"];
    menu = "MENU_EDIT";
    startListening();
}
...
loadConfig() {
    ...
    for(i = 0;i<osGetNumberOfNotecardLines(gs_ConfigName); i++) {
        string line = osGetNotecardLine(gs_ConfigName, i);
        lParams = llParseString2List(line, ["="], [""]);
        if (llGetListLength(lParams) == 2){
            ...
            } else if ((sParam == "autoOn")||(sParam == "AutoOn")) {
                autoOn = (sValue == "TRUE");
            ...
            }else if ((sParam == "ownerOnlyRemote")||(sParam == "OwnerOnlyRemote")) {
                ownerOnlyRemote = (sValue == "TRUE");
            }else if ((sParam == "silent")||(sParam == "Silent")) {
                silent = (sValue == "TRUE");
            }else if ((sParam == "allowRemoteControl")||(sParam == "AllowRemoteControl")) {
                allowRemoteControl = (sValue == "TRUE");
            }else {
...
default
{
    state_entry() {
        ...
        if (llListFindList(invGroups,[defaultGroup]) == -1) {
            string msgToSay = "ERROR! Unable to find the specified default group \""+defaultGroup+"\" in inventory. Make sure you supplied the simple group name, not the full card name.";
            if (llGetListLength(invGroups) == 0)
            {
            ...
            {
                defaultGroup = llList2String(invGroups,0);
        ...
        if (change & CHANGED_LINK) {
            ...
            while (i>l) {
                ...
                if ((myState == "ERROR") || (myState == "INITIALIZING")) {
                    ...
                    if (llListFindList(positions,[who]) == -1) { // new sitter
            ...
            if (!realAvi) {
                if (diaHandle && !allowRemoteControl) {
    ...
    touch_start(integer num) {
        ...
        } else if ((myState == "RUNNING")||(myState == "READY")) {
            if ((myState == "READY")&&(!allowRemoteControl)) {
                llRegionSayTo(who,0,"Please sit to begin using me");
                return;
            }
            if ((allowRemoteControl == FALSE) && (llListFindList(positions,[who]) == -1)) {
                llRegionSayTo(who,0,"Only current users may access the controls. Please sit, then try again");
                return;
            }
            if (ownerOnlyMenus && (who != llGetOwner())) {
                if (!silent) llRegionSayTo(who,0,"Sorry, this item is currently set to only allow the owner to access the controls.");
                return;
            }
            if (who != user) {
                if (user == NULL_KEY) {
    ...
    listen (integer channel, string name, key who, string message) {
        if (who != user) {
            if (message == "TAKE CONTROL") {
                if (
                        (myState == "RUNNING")||((myState == "READY") && allowRemoteControl)) doChangeUser(who);
                else {
            ...
            if (message == "CANCEL") showOptionsMenu();
                ...
                key npc = osNpcCreate(llList2String(thisNames, 0), llList2String(thisNames, 1), llGetPos() + <0.0,0.0,2.0>, npcToLoad, 8 | OS_NPC_SENSE_AS_AGENT);
            ...
            if (message == "CANCEL") showOptionsMenu();
                ...
                showSpecialsMenu();
            } else llMessageLinked(LINK_THIS,0,llList2String(specials,llListFindList(specials,[message])+1)+"|"+ (string)user,llDumpList2String(positions,"|"));
            ...
            else if (message == "SWAP ANS") {
                if (llGetListLength(positions) != 2) llOwnerSay("Sorry, Swap in edit mode only works for couples poses");
                else {
                    key av0 = llList2Key(positions,0);
                    key av1 = llList2Key(positions,1);
                    key hand0 = llList2Key(editHandles,0);
                    key hand1 = llList2Key(editHandles,1);
                    list loc0 = llGetObjectDetails(hand0,[OBJECT_POS,OBJECT_ROT]);
                    list loc1 = llGetObjectDetails(hand1,[OBJECT_POS,OBJECT_ROT]);
                    osSetPrimitiveParams(hand0,[PRIM_POSITION,llList2Vector(loc1,0),PRIM_ROTATION,llList2Rot(loc1,1)]);
                    osSetPrimitiveParams(hand1,[PRIM_POSITION,llList2Vector(loc0,0),PRIM_ROTATION,llList2Rot(loc0,1)]);
                    osAvatarStopAnimation(av0,llList2String(currentAn,3));
                    osAvatarPlayAnimation(av0,llList2String(currentAn,6));
                    osAvatarStopAnimation(av1,llList2String(currentAn,6));
                    osAvatarPlayAnimation(av1,llList2String(currentAn,3));
                    currentAn = []+llList2List(currentAn,0,2)+llList2List(currentAn,6,8)+llList2List(currentAn,3,5);
                }
            }
            else {
                persistChanges();
                if (message == "EDIT OFF") removeHandles();
                else if ((message == "DEL ANIM")||(message == "STORE THIS") || (message == "< PREV") || (message == "NEXT >")) {
                    integer anToPlay = llListFindList(anList,[llList2String(currentAn,1)]);
                    if (message == "< PREV") anToPlay--;
                    else if (message == "NEXT >") anToPlay++;
                    else if (message == "DEL ANIM") {
                        anList = []+llDeleteSubList(anList,anToPlay,anToPlay);
                        integer anIndex = llListFindList(anData,llList2List(currentAn,1,1));
                        anData = []+llDeleteSubList(anData,anIndex,anIndex+anStride-1);
                    }
